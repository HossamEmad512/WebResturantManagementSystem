...
export class OrdersComponent implements OnInit {
  ...
  creatsales(Data: any, id: any): void {
    ...
    this._RecoomendationsService.AddOrderItem(Data).subscribe((r) => {
      console.log(r);
    });
    var orderItems: any = [];
    ...
    for (let index = 0; index < Data.orderItems.length; index++) {
      var Item = {
        item: {
          name: Data.orderItems[index].item.name,
          cost: Data.orderItems[index].item.cost,
          price: Data.orderItems[index].item.price,
        },
