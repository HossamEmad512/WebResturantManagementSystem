...
export class CreateOrdersComponent implements OnInit {
  ...
  addOrderDetails(item: any): void {
    ...
    var order = {
      item: { name: item.name, cost: item.cost, price: item.price },
      quantity: this.formgrioup.get('quantity')?.value,
    };
    this.orderItems.push(order);
    this.formgrioup.get('quantity')?.setValue('');
    console.log(this.orderItems);
  }
  // إظهار النموذج
  showForm() {
    this.isFormVisible = true;
    let x = this.orderItems;
    console.log(x);
  }
